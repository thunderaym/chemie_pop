...
var box, tutorial_board, tutorial_btn, tutorial_s, 	tutorial_screen, info_mask;
var card, skip_btn;
...
function FIRST_TIME_USER() {
	info_mask = game.add.sprite(0, 0, 'black-fade');
	info_mask.alpha = 0;
	info_mask.scale.setTo(.5);
	tutorial_board = game.add.sprite(game.world.centerX, -230, 'tutorial_board');
	tutorial_board.scale.setTo(.5);
	tutorial_board.anchor.setTo(.5);
	var x_btn = game.add.button(305, -403, 'x_btn', CLOSE_TUTORIAL_SCREEN, this);
	x_btn.anchor.setTo(.5);
	tutorial_board.addChild(x_btn);
	tutorial_btn = game.add.button(0, 354, 'tutorial_btns', NEXT_TUTORIAL_SCREEN, this);
	tutorial_btn.anchor.setTo(.5);
	tutorial_board.addChild(tutorial_btn);
	tutorial_s = game.add.group();
	for (var i=1;i<4;i++) {
		var t_s = tutorial_s.create(0, -347, 'tutorial_s'+i);
		t_s.anchor.setTo(.5, 0);
		t_s.alpha = !Math.floor(i/2);
	}
	tutorial_board.addChild(tutorial_s);
	
	game.add.tween(info_mask).to({alpha: .9},500,Phaser.Easing.Exponential.InOut,true);
	var tween = game.add.tween(tutorial_board).to({y: game.world.centerY},1000,Phaser.Easing.Exponential.InOut,true);
}

function CLOSE_TUTORIAL_SCREEN(btn) {
	game.add.tween(btn.scale).to({x: 0.9, y: 0.9}, 150, Phaser.Easing.Exponential.InOut, true, 0, 0, true);
	game.add.tween(info_mask).to({alpha: 0},500,Phaser.Easing.Exponential.InOut,true,500);
	var tween = game.add.tween(tutorial_board).to({y: -230},1000,Phaser.Easing.Exponential.InOut,true,400);
	tween.onComplete.add(function() {
		tutorial_board.destroy(true);
		TUTORIAL_START();
		TUTORIAL_PROTONS();
		TUTORIAL_HINT();
		TUTORIAL_POP();
...
function NEXT_TUTORIAL_SCREEN(btn) {
	game.add.tween(btn.scale).to({x: 0.9, y: 0.9}, 150, Phaser.Easing.Exponential.InOut, true, 0, 0, true);
	if (tutorial_screen==3) {
		...
		tutorial_screen++;
		setTimeout(function(){
			tutorial_btn.frame = Math.floor(tutorial_screen/3);
		}, 400);			
		for (var i=1;i<4;i++) {
			var ss = tutorial_s.getAt(i-1);
			if (i==tutorial_screen) {
				game.add.tween(ss).to({alpha: 1},300,Phaser.Easing.Exponential.InOut,true,400);
			}
			else {
				game.add.tween(ss).to({alpha: 0},300,Phaser.Easing.Exponential.InOut,true,400);
			}
		}
	}	
